...
export default function Payment({
    ...
    useFlashToast();
    const { activeOutlet, security } = usePage().props;
        ...
        <>
            ...
            <form onSubmit={handleSubmit} className="max-w-3xl space-y-5">
                ...
                <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 p-5">
                    ...
                    {data.bank_transfer_enabled && (
                        <p className="text-xs text-slate-400 dark:text-slate-500 flex items-start gap-1 mb-3">
                            <span className="mt-0.5 shrink-0">*</span>
                            Pastikan rekening bank sudah diatur sebelum mengaktifkan metode ini.
                        </p>
                    )}
                    <a
                    ...
                    >
                        {data.midtrans_enabled && (
                            <p className="text-xs text-slate-400 dark:text-slate-500 flex items-start gap-1">
                                <span className="mt-0.5 shrink-0">*</span>
                                Server key dan Client key wajib diisi. Dapatkan dari dashboard Midtrans → Settings → Access Keys.
                            </p>
                        )}
                        <div className="grid gap-4 md:grid-cols-2">
                            <Input
                                label="Server Key"
                                required={data.midtrans_enabled}
                                type="password"
                                ...
                                hintText="Server Key dari dashboard Midtrans. Bersifat rahasia, jangan dibagikan. Jika dikelola via env file, tidak bisa diubah dari sini."
                            />
                            <Input
                                label="Client Key"
                                required={data.midtrans_enabled}
                                ...
                                disabled={!canUpdatePaymentSettings}
                                hintText="Client Key dari dashboard Midtrans. Bersifat publik, aman ditampilkan di frontend."
                            />
                        </div>
                        {renderSecretHint(
                            "midtrans_server_key",
                        ...
                        <p className="ml-6 text-xs text-slate-400 dark:text-slate-500 flex items-start gap-1">
                            <span className="mt-0.5 shrink-0">*</span>
                            Centang jika sudah siap menerima pembayaran nyata. Biarkan tidak tercentang untuk mode sandbox/pengembangan.
                        </p>
                    </div>
                </div>
                ...
                <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 p-5">
                    ...
                    >
                        {data.xendit_enabled && (
                            <p className="text-xs text-slate-400 dark:text-slate-500 flex items-start gap-1">
                                <span className="mt-0.5 shrink-0">*</span>
                                Secret Key, Public Key, dan Callback Token wajib diisi. Dapatkan dari dashboard Xendit → Settings → API Keys.
                            </p>
                        )}
                        <div className="grid gap-4 md:grid-cols-2">
                            <Input
                                label="Secret Key"
                                required={data.xendit_enabled}
                                type="password"
                                ...
                                hintText="Secret Key dari dashboard Xendit. Bersifat rahasia, jangan dibagikan."
                            />
                            <Input
                                label="Public Key"
                                required={data.xendit_enabled}
                                ...
                                disabled={!canUpdatePaymentSettings}
                                hintText="Public Key dari dashboard Xendit. Bersifat publik, aman ditampilkan di frontend."
                            />
                        </div>
                        {renderSecretHint(
                            "xendit_secret_key",
                        ...
                        <Input
                            ...
                            hintText="Callback Token untuk verifikasi webhook Xendit. Harus sama dengan yang diatur di dashboard Xendit → Settings → Callbacks."
                        />
                        {renderSecretHint(
                            "xendit_callback_token",
                    ...
                    {webhookWarnings.length > 0 && (
                        <div className="mb-4 space-y-2">
                            {webhookWarnings.map((warning) => (
                                <div
                                    key={warning}
                                    className="rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-xs text-amber-800 dark:border-amber-900/60 dark:bg-amber-950/30 dark:text-amber-300"
                                >
                                    {warning}
                                </div>
                            ))}
                        ...
                        <div>
                            ...
                            <div className="flex items-center gap-2">
                                <input
                                    ...
                                    className="flex-1 h-10 px-3 text-xs rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 text-slate-600 dark:text-slate-400"
                                />
                                <button
                                    type="button"
                                    onClick={() => {
                                    ...
                                    className="px-3 h-10 rounded-lg border border-slate-200 dark:border-slate-700 text-xs font-medium text-slate-600 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800"
                                >
                                    Salin
                                </button>
                            </div>
                            <p className="mt-1 text-[10px] text-slate-400 dark:text-slate-500">
                                Paste ke dashboard Midtrans → Settings → Notification URL.
                            </p>
                        </div>
                        <div>
                            <label className="block text-xs font-medium text-slate-600 dark:text-slate-400 mb-1">
                            ...
                            <p className="mt-1 text-[10px] text-slate-400 dark:text-slate-500">
                                Paste ke dashboard Xendit → Settings → Callbacks.
                            </p>
                        </div>
                    </div>
                </div>
                ...
                <div className="flex justify-end">
                    <button
                        ...
                        className="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl bg-primary-600 hover:bg-primary-700 text-white text-sm font-semibold transition-colors disabled:opacity-50"
                    >
            ...
            <ConfirmPasswordModal
                ...
                challengeLabel="menyimpan konfigurasi payment gateway"
