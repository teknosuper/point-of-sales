...
3. **Upload "Entity Too Large" (413) is a transport limit, not Laravel validation** — `php artisan serve` uses the CLI ini (`/etc/php/<ver>/cli/php.ini`), default `post_max_size=8M`, `upload_max_filesize=2M`. A request body > `post_max_size` is rejected by PHP **before** Laravel runs (raw "Entity Too Large"/413); a file > `upload_max_filesize` is **silently dropped** (`$_FILES` error=1) so Laravel's `required/max:2048` won't fire and the product silently saves `default.jpg`. App-side validation (`ProductController image max:2048`) is correct but can only run if the transport lets the file through. Keep the frontend guard in `resources/js/Utils/imageUpload.js` (`IMAGE_UPLOAD_MAX_BYTES`=2MB, `MAX_ORIGINAL_UPLOAD_BYTES`=7MB) in sync, and on servers raise `upload_max_filesize`/`post_max_size` (and nginx `client_max_body_size`) to well above the frontend cap so oversize files reach Laravel and fail with a friendly validation error instead of 413/500.
